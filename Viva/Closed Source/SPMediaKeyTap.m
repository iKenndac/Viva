...
static CGEventRef tapEventCallback2(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon)
{
	...
	if (![self shouldInterceptMediaKeyEvents])
	 ...
	 // matched in handleAndReleaseMediaKeyEvent:
	[self performSelectorOnMainThread:@selector(handleMediaKeyEvent:) withObject:nsEvent waitUntilDone:YES];
...
-(void)appIsNowFrontmost:(ProcessSerialNumber)psn;
{
	...
	NSDictionary *processInfo = (__bridge_transfer id)ProcessInformationCopyDictionary(
		&psn,
		kProcessDictionaryIncludeAllInformationMask
	);
	NSString *bundleIdentifier = [processInfo objectForKey:(id)kCFBundleIdentifierKey];
